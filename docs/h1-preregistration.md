...
**A9. Positive definition, matching key, subsystem quota (amends §3).** (i) A positive is a PR whose corrective evidence the evaluator **confirms by reading the fixing commit(s)**: the fix corrects a defect the PR introduced or newly exposed, and the escape is system-level under §7. The sampler's tier (strong, or medium `SAME_FUNCTION_FIX`) selects the *pool*; it is not the definition — 3 of 7 strong-tier positives in 2026H1 were sampler artefacts (a backport merge, a feature withdrawal), and 2 of 214 medium-tier candidates were real. (ii) Matching key relaxed to **(stateful category, subsystem)**; the changed-lines band is recorded as a covariate, not matched (`cohort-export --match-key`, cohort-manifest/v4). (iii) **No subsystem supplies more than 8 of the 20 positives.** (iv) Negative reads and the security-adjacent exclusion are unchanged. (v) **Subsystem keying is code-first** (added 2026-09-16 after the evaluator found topotest-heavy PRs keyed `tests`): the primary subsystem is the code subsystem with the most changed lines, ignoring `tests/`, `doc/`, `tools/`, `.github/` and top-level build files; a PR touching nothing else keeps `tests`/`doc`. Applies to positives, negatives and arm H alike; recorded in the cohort manifest as `subsystem_rule`. (vi) **Fallback matching** (added 2026-09-16 at cohort construction): a positive with no counted-clean negative in its (category, subsystem) cell is matched on **subsystem alone**, taking the negative with the nearest category (HIGH→MEDIUM→LOW) and then the nearest size band; the pair records `match_key_used: [subsystem]` and the manifest counts `fallback_pairs`. Under fallback the stateful category is unbalanced within the pair on a dimension the arms can see in the diff (bgpd clean negatives are all MEDIUM, so every HIGH bgpd positive is a fallback pair by construction). Fallback pairs are a reported stratum; §2's pairwise deltas are given with and without them. A positive with no negative in its subsystem at all stays unmatched and is excluded, and the cohort doc says so. (vii) **Backport PRs (Mergify-generated stable-branch backports) are excluded from both classes** (added at cohort construction, 2026-09-16): a backport carries the same diff as its master original with no review of its own, so it is not an independent change; 208 of 617 candidate negatives were backports. The frozen cohort's negatives were picked by the evaluator from the counted-clean pool under (v)–(vi) with a global min-cost assignment (exact size band preferred), recorded explicitly in the manifest (`filters.negatives`); the sampler's own picks are not used.
...
**A12. Two rulings applying §3 as written, recorded so they bind.** (i) A fixing commit that exists only on an unmerged `refs/pull/*` head is **not corrective evidence**; the correlator currently counts such commits (~25% of fixing SHAs in the medium tier, 1 of 7 strong positives in 2026H1) and a `--require-reachable` filter is backlog. A case whose only fix is unmerged is not admitted; it is re-admitted if the fix merges before freeze. (ii) A security-adjacent negative is excluded even when a reviewer flagging it would be wrong (i.e., even when it is a *good* negative) — §3's rule stands; changing it would be a v2 decision. (iii) **A daemon crash or abort triggered by an operator action** (a `show` command, a reconfigure, an instance teardown) **is a runtime failure and admissible** as `other`, or as `classical-memory-safety` where §3's routing/state-path exception applies (ruling of 2026-09-16, applied uniformly across windows). (iv) A positive for which no honest Probe A symptom can be written (no symptom or mechanism stated anywhere offline, e.g. a same-day CI revert) is **not admitted**, because A11 requires the probe for every positive.
